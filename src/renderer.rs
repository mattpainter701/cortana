...
impl FaceRenderer {
    ...
    pub fn paint_on_canvas(
        ...
        self.draw_hair_silhouette(ctx, tilt, breathe, fc);
        self.draw_cortana_crown(ctx, tilt, breathe, arousal, fc);
    ...
    fn draw_aura_rings(&self, ctx: &mut ratatui::widgets::canvas::Context, tilt: f64, breathe: f64, arousal: f64, fc: u64) {
        ...
        for _ in 0..rate {
            if particles.len() < 60 {
                ...
                particles.push(Particle {
                    ...
                    vx: (0.5 - rand_f64()) * 0.6, vy: -0.3 - rand_f64() * 0.7,
                    life: 0.0, max_life: 15.0 + rand_f64() * 40.0,
                });
            }
        }
        let mut i = 0;
    ...
    fn draw_particles(&self, ctx: &mut ratatui::widgets::canvas::Context, particles: &[Particle]) {
        ...
        for p in particles {
            ...
            let _a = if frac > 0.7 { (1.0 - frac) / 0.3 * 0.7 } else { 0.7 };
            if frac < 0.25 { bright.push((p.x, p.y)); }
            else if frac < 0.55 { mid.push((p.x, p.y)); }
    ...
    // ── face fill (heart-shaped hologram) ──

    fn draw_face_fill(&self, ctx: &mut ratatui::widgets::canvas::Context, tilt: f64, breathe: f64, fc: u64) {
        // Heart-shaped face: wider at temples, narrows to chin
        // Upper face radius, lower face radius
        ...
        for iy in -16..=16i32 {
            let y = iy as f64 * 2.0;
            let t = (y + ry_face) / (2.0 * ry_face); // 0 at top, 1 at bottom
            let rx = rx_top + (rx_bot - rx_top) * t;
            let nx = ((y / ry_face).powi(2)).min(1.0);
            let spread = rx * (1.0 - nx).sqrt();

            let density = if iy < -8 { 4 } else if iy > 10 { 3 } else { 5 };
    ...
    fn draw_jawline(&self, ctx: &mut ratatui::widgets::canvas::Context, tilt: f64, breathe: f64, fc: u64) {
        ...
        // Draw heart-shaped contour
        let n = 100;
        let mut pts = Vec::with_capacity(n);
        for i in 0..n {
            let a = TAU * i as f64 / n as f64;
            // Heart shape: upper wider, lower narrower
            let sin_a = a.sin();
            let t = (sin_a + 1.0) / 2.0; // 0 at top, 1 at bottom
            let rx = rx_top + (rx_bot - rx_top) * t;
            let x = a.cos() * rx;
            let y = cy + sin_a * ry;
        ...
        for i in 0..50 {
            ...
            let sin_a = a.sin();
            let t = (sin_a + 1.0) / 2.0;
            let rx = (rx_top - 3.0) + (rx_bot - 2.0 - rx_top + 3.0) * t;
            let x = a.cos() * rx;
    ...
    fn draw_neck(&self, ctx: &mut ratatui::widgets::canvas::Context, tilt: f64, breathe: f64, fc: u64) {
    ...
    // ── hair silhouette (sides of face) ──

    fn draw_hair_silhouette(&self, ctx: &mut ratatui::widgets::canvas::Context, tilt: f64, breathe: f64, fc: u64) {
        let shim = (fc as f64 * 0.02).sin() * 0.03 + 0.12;
        let hair_c = Color::Rgb(10, 45, 110);
        // Hair flares out from the sides of the face
        let mut pts = Vec::new();
        for side in [-1.0, 1.0] {
            for i in -8..=12i32 {
        ...
        self.dots(ctx, &pts, hair_c, shim);
    }

    // ── CORTANA CROWN (signature feature) ──

    fn draw_cortana_crown(&self, ctx: &mut ratatui::widgets::canvas::Context, tilt: f64, breathe: f64, arousal: f64, fc: u64) {
        let active = arousal * 0.5 + 0.4;
        let pulse = (fc as f64 * 0.03).sin() * 0.08 + 1.0;

        // 5 overlapping arch bands — Cortana's signature crown
        // Each band is a sweeping arc from temple to temple over the head
        for layer in 0..5i32 {
            ...
            for sub in 0..sublayers {
                ...
                for i in 0..=segs {
                    ...
                    let angle = -0.75 + t * 1.5; // -0.75 to 0.75 radians ~  -43 to 43 deg
                    let x = angle.sin() * span;
                    // Parabolic arch
                    let arch = (angle * 1.6).cos() * arch_height;
                    let y = by + arch + sub_offset;
                    // Only draw the upper portion (the crown arcs)
                    if y < by + arch_height * 0.3 {
                        pts.push(self.tf(x, y, tilt, breathe));
                    }
                }
                let crown_color = match layer {
                    0 => Color::Rgb(60, 200, 255),
                    1 => Color::Rgb(45, 170, 240),
                    2 => Color::Rgb(35, 145, 225),
                    3 => Color::Rgb(25, 120, 210),
                    _ => Color::Rgb(15, 95, 195),
                };
                self.dots(ctx, &pts, crown_color, alpha * (1.0 - sub as f64 * 0.2));
            }
        }

        // Crown glow aura
        let glow_alpha = active * 0.15 * pulse;
        let mut glow_pts = Vec::new();
        for layer in 0..3i32 {
            ...
            for i in 0..30 {
                ...
                for dx in [-2.0, 0.0, 2.0] {
                    for dy in [-2.0, 0.0, 2.0] {
                        glow_pts.push(self.tf(x + dx, y + dy, tilt, breathe));
                    }
                }
            }
        }
        self.dots(ctx, &glow_pts, Color::Rgb(30, 130, 220), glow_alpha * 0.08);
    }
    ...
    fn draw_eyes(&self, ctx: &mut ratatui::widgets::canvas::Context, tilt: f64, breathe: f64, gaze_x: f64, gaze_y: f64, params: &FaceParams, fc: u64) {
        ...
        for sx in [-11.0, 11.0f64].iter() {
            ...
            for ix in -3..=3i32 {
                for iy in -3..=3i32 {
                    ...
                    if dx * dx * 1.2 + dy * dy <= 4.0 {
                        iris.push(self.tf(sx + gx + dx * 1.2, eye_y + gy + dy * 0.9, tilt, breathe));
                    }
                }
            }
            ...
            for ix in -1..=1i32 {
                for iy in -1..=1i32 {
                    if ix * ix + iy * iy <= 1 {
                        pupil.push(self.tf(sx + gx + ix as f64 * 0.9, eye_y + gy + iy as f64 * 0.7, tilt, breathe));
                    }
                }
            }
    ...
    fn draw_brows(&self, ctx: &mut ratatui::widgets::canvas::Context, tilt: f64, breathe: f64, params: &FaceParams) {
        ...
        for sx in [-11.0, 11.0f64].iter() {
            ...
            self.dots(ctx, &pts, Color::Rgb(60, 175, 245), 0.5 + raise.abs() as f64 * 0.15);
        }
    }
    ...
    fn draw_mouth(&self, ctx: &mut ratatui::widgets::canvas::Context, tilt: f64, breathe: f64, params: &FaceParams, _valence: f64) {
        ...
        self.dots(ctx, &upper, Color::Rgb(75, 195, 250), 0.55 + smile.abs() * 0.1);
        ...
        if open < 0.04 {
            ...
            self.dots(ctx, &lower, Color::Rgb(60, 170, 235), 0.35 + smile.abs() * 0.05);
        }
    ...
    fn draw_collar(&self, ctx: &mut ratatui::widgets::canvas::Context, tilt: f64, breathe: f64, fc: u64) {
        let shim = (fc as f64 * 0.06).sin() * 0.08;
        let edge_c = Color::Rgb(55, 155, 235);
        let dim_c = Color::Rgb(15, 50, 130);

        // Collar V-neck shape
        let pts = [(-19.0, 42.0), (-7.0, 28.0), (0.0, 23.0), (7.0, 28.0), (19.0, 42.0)];
        for i in 0..4 {
    ...
    fn draw_cheek_glow(&self, ctx: &mut ratatui::widgets::canvas::Context, tilt: f64, breathe: f64, params: &FaceParams) {
        let arousal = params.arousal as f64;
        if arousal < 0.1 { return; }
...
pub mod kitty {
    use crate::face::FaceParams;
    use tiny_skia::{Pixmap, Paint, Path, PathBuilder, Stroke, Transform, Color as SkColor, FillRule};
    use std::io::Write;
    ...
    fn mk_stroke(w: f64) -> Stroke {
        Stroke { width: w as f32, ..Stroke::default() }
    }

    fn draw_fill(pix: &mut Pixmap, path: &Path, r: u8, g: u8, b: u8, a: f64) {
        pix.fill_path(path, &mk_paint(r, g, b, a), FillRule::Winding, Transform::identity(), None);
    }

    fn draw_stroke(pix: &mut Pixmap, path: &Path, r: u8, g: u8, b: u8, a: f64, w: f64) {
        pix.stroke_path(path, &mk_paint(r, g, b, a), &mk_stroke(w), Transform::identity(), None);
    }
    ...
    pub fn render_pixmap(params: &FaceParams, width: u32, height: u32) -> Pixmap {
            ...
            for layer in 0..5i32 {
                ...
                for sub in 0..sublayers {
                    ...
                    if let Some(path) = pb.finish() {
                        ...
                        draw_stroke(p, &path, crown_color.0, crown_color.1, crown_color.2, alpha * (1.0 - sub as f64 * 0.2), 1.8);
                    }
                }
            }
        ...
        // ═══════════════ Eyes ═══════════════
        {
            ...
            for sx in [-11.0, 11.0] {
                ...
                // Pupil
                draw_circle(p, sx + gx, eye_y + gy, 1.2, 160, 230, 255, 0.7 * shimmer_eye);
                // Highlights
                draw_circle(p, sx + gx + 2.0, eye_y + gy - eh * 0.5, 0.8, 200, 240, 255, 0.85 * shimmer_eye);
                draw_circle(p, sx + gx + 0.5, eye_y + gy - eh * 0.8, 0.5, 180, 235, 255, 0.6 * shimmer_eye);
            }
        }
        ...
        // ═══════════════ Mouth ═══════════════
        {
            ...
            for i in 0..24 {
                ...
                let mu = my - smile * 2.2 * (1.0 - ((t - 0.5) * 2.0).powi(2)) - bow_dip;
                let (px, py) = tf(mx, mu);
                if first { ub.move_to(px, py); first = false; }
                else { ub.line_to(px, py); }
            }
            ...
            if open < 0.04 {
                ...
                for i in 0..18 {
                    ...
                    let ml = my + 2.5 + smile * 1.5 * (1.0 - ((t - 0.5) * 2.0).powi(2));
                    let (px, py) = tf(mx, ml);
                    if first { lb.move_to(px, py); first = false; }
                    else { lb.line_to(px, py); }
                }
            ...
            if open > 0.03 {
                ...
                for i in 0..18 {
                    ...
                    let ml = my + gap * (1.0 - ((t - 0.5) * 1.4).abs().powi(2));
                    let (px, py) = tf(mx, ml);
                    if first { lb2.move_to(px, py); first = false; }
                    else { lb2.line_to(px, py); }
                }
...
mod tests {
    ...
    fn backend_from_string_maps_correctly() {
        ...
        assert_eq!(RendererBackend::from_str("unknown"), RendererBackend::Canvas);
    }
}
